//Credit: https://github.com/AnikSarker/ACM-Library/
...
double get_Area(double ha, double hb, double hc) {
    ...
    double _A_ = 4 * sqrt(H * (H - 1 / ha) * (H - 1 / hb) * (H - 1 / hc));
    return 1.0 / _A_;
}
...
void rotatingCalipersGetRectangle(Point* p, int n, double& area,
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (dcmp(getDot(v, p[r % n] - p[i]) -
                    getDot(v, p[(r + 1) % n] - p[i])) < 0)
            r++;
        while (j < r || dcmp(getCross(v, p[j % n] - p[i]) -
                             getCross(v, p[(j + 1) % n] - p[i])) < 0)
            j++;
        while (l < j || dcmp(getDot(v, p[l % n] - p[i]) -
                             getDot(v, p[(l + 1) % n] - p[i])) > 0)
            l++;
...
Polygon cutPolygon(Polygon u, Point a, Point b) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (dcmp((b - a) * (d - c)) != 0) {
            ...
            if (onSegment(t, c, d)) ret.push_back(t);
        }
    }
    return ret;
}
...
int pointInConvexPolygon(Point* pt, int n, Point p) {
    ...
    while (hi - lo > 1) {
        int mid = (lo + hi) / 2;
        if (getCross(pt[mid] - pt[0], p - pt[0]) > 0)
            lo = mid;
        else
            hi = mid;
    }
...
int getLineCircleIntersection(Point p, Point q, Circle O, double& t1,
...
double areaCircleTriIntersection(Circle c, Segment s) {
    ...
    if (dcmp(getDistanceToSegment(c.o, s.a, s.b) - c.r) >= 0)
        return fix_acute(getSignedAngle(s.a - c.o, s.b - c.o)) * (c.r * c.r) /
               2.0;
...
int getCircleCircleIntersection(Circle o1, Circle o2, vector<Point>& sol) {
    ...
    Vector v = o2.o - o1.o;
    double co =
        (o1.r * o1.r + getPLength(v) - o2.r * o2.r) / (2 * o1.r * getLength(v));
...
double areaCircleCircle(Circle o1, Circle o2) {
    ...
    double alpha1 =
        acos((o1.r * o1.r + d * d - o2.r * o2.r) / (2.0 * o1.r * d));
    double alpha2 =
        acos((o2.r * o2.r + d * d - o1.r * o1.r) / (2.0 * o2.r * d));
...
int getTangents(Circle o1, Circle o2, Point* a, Point* b) {
    ...
    if (dcmp(d2 - rsum * rsum) == 0) {
        a[cnt] = o1.point(base);
        b[cnt] = o2.point(PI + base);
        cnt++;
    } else if (dcmp(d2 - rsum * rsum) > 0) {
        double ang = acos((o1.r + o2.r) / sqrt(d2));
        a[cnt] = o1.point(base + ang);
        b[cnt] = o2.point(PI + base + ang);
        cnt++;
        a[cnt] = o1.point(base - ang);
        b[cnt] = o2.point(PI + base - ang);
        cnt++;
...
Circle CircumscribedCircle(Point p1, Point p2, Point p3) {
    ...
    double cx =
        (Cy * (Bx * Bx + By * By) - By * (Cx * Cx + Cy * Cy)) / D + p1.x;
    double cy =
...
Circle getApolloniusCircle(const Point& P, const Point& Q, double rp,
                           double rq) {
    rq *= rq;
    rp *= rp;
    double a = rq - rp;
    ...
    h /= a;
    double c =
        rq * P.x * P.x - rp * Q.x * Q.x + rq * P.y * P.y - rp * Q.y * Q.y;
    c /= a;
    Point o(g, h);
    double R = g * g + h * h - c;
    R = sqrt(R);
